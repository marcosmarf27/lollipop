...
class CustomerFormView extends TPage
{
    ...
    function __construct( $param, $embedded = false)
    {
       ...
       // $this->form->setClientValidation(true);
        ...
        $status->addItems( [ 'S' => 'Ativo', 'N' => 'Desativado'] );
        ...
        $this->form->addFields( [ new TLabel('Cod') ],      [ $code ] );
        $this->form->addFields( [ new TLabel('Nome') ],      [ $name ] );
        $this->form->addFields( [ new TLabel('Endereço') ],   [ $address ] );
        $this->form->addFields( [ new TLabel('Cidade') ],      [ $city_id ] );
        $this->form->addFields( [ new TLabel('Celular') ],     [ $phone ],
                                [ new TLabel('Aniversário') ], [ $birthdate ] );
        $this->form->addFields( [ new TLabel('Status') ],    [ $status ],
                                [ new TLabel('Email') ],     [ $email ]);
        $this->form->addFields( [ new TLabel('Categoria') ],  [ $category_id ],
                                [ new TLabel('Genero') ],    [ $gender ] );
        
        $this->form->appendPage('Preferências');
        $skill_list = new TDBCheckGroup('skill_list', 'samples', 'Skill', 'id', 'name');
        $this->form->addFields( [ new TLabel('Preferências') ],     [ $skill_list ] );
        ...
        $this->contacts->addField( '<b>Tipo</b>', $contact_type, ['width' => '50%']);
        $this->contacts->addField( '<b>Valor</b>', $contact_value, ['width' => '50%']);
