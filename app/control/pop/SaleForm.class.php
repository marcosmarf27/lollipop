...
class SaleForm extends TWindow
{
    ...
    function __construct()
    {
        ...
        $this->form->setFormTitle('Pedido detalhes');
       ...
       // $date        = new TDate('date');
        ...
        $obs         = new TText('obs');
        $obs->placeholder = 'Use <BR> para pular linhas. (exemplo TROCO PARA R$ 50 <BR>  NÃO COLOCAR CEBOLA <BR>)';
        $obs->forceUpperCase();
        $status  =  new TCombo('status');
        $pagamento  =  new TCombo('pagamento');
      

        $pagamento_itens = ['1'=>'Dinheiro', '2'=>'Cartão Débito', '3'=>'Cartão de Crédito', '4' => 'Pix'];
        $status_itens = ['1'=>'Aberto', '2'=>'Pago', '3'=>'Cancelado'];

        $status->addItems($status_itens);
        $pagamento->addItems($pagamento_itens);
      ...
      //  $date->addValidation('Date', new TRequiredValidator);
        ...
        $this->form->addFields( [new TLabel('Status')], [$status], [new TLabel('Pagamento')], [$pagamento] );
        
        $this->form->addContent( ['<h4>ITENS DO PEDIDO</h4><hr>'] );
        ...
        $add_product = TButton::create('add_product', [$this, 'onProductAdd'], 'Registrar item', 'fa:plus-circle green');
        ...
        $col_pid    = new TDataGridColumn( 'product_id', 'Nº pedido', 'center', '10%');
        $col_descr  = new TDataGridColumn( 'product_id', 'Descrição', 'left', '30%');
        $col_amount = new TDataGridColumn( 'amount', 'quantidade', 'left', '10%');
        ...
        $customer_id->setMask('<b>NOME:</b> {name}  <b>ENDEREÇO: </b> {address}');
    ...
    public function onProductAdd( $param )
    {
        try
        {
            ...
            {
                throw new Exception('Campos descrição, valor, quantidade são obrigatórios!');
            }
    ...
    public function onSave($param)
    {
        try
        {
            ...
            $action1 = new TAction(array('ImpressaoWindow', 'onPrint'));
            $action2 = new TAction(array('SaleList', 'onReload'));
            // define os parâmetros de cada ação
            $action1->setParameter('key', $sale->id);
            $action2->setParameter('parkeyameter', 2);
            
            // shows the question dialog
            new TQuestion('Gostaria de imprimir o pedido ?', $action1);

         /*    $acao = new TAction(array('SaleList', 'onReload'));
    ...
    {
        parent::closeWindow();
        AdiantiCoreApplication::loadPage('SaleList', 'onReload');
    }
}
