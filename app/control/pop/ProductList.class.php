...
class ProductList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->addFilterField('id', '=');            // add a filter field
        ...
        $id        = new TEntry('id');
        //$unit->addItems( ['kg' => 'kg', 'und' => 'und'] );
        $unit->addItems( ['pizza' => 'Pizzas', 'hamburguer' => 'Hamburguers', 'esfihas' => 'Esfihas', 'cerveja' => 'Cervejas', 'refrigerante' => 'Refrigerantes',  'bebidas' => 'Outras bebidas', 'sucos' => 'Sucos', 'milk' => 'Milk Shakes', 'adicional' => 'Adicionais', 'petisco' => 'Petiscos', 'outros' => 'Outros'] );
        
        // add a row for the filter field
        $this->form->addFields( [new TLabel('Descrição')], [$description] );
        $this->form->addFields( [new TLabel('Categoria')], [$unit] );
        $this->form->addFields( [new TLabel('Cod. produto')], [$id] );
        
        $this->form->setData( TSession::getValue('ProductList_filter_data') );
        
        $this->form->addAction( 'Buscar', new TAction([$this, 'onSearch']), 'fa:search blue');
        $this->form->addActionLink( 'Novo',  new TAction(['ProductForm', 'onEdit']), 'fa:plus green');
        ...
        $col_description = new TDataGridColumn('description', 'Descrição', 'left', '60%');
      //  $col_stock       = new TDataGridColumn('stock', 'Estoque', 'right', '15%');
        $col_sale_price  = new TDataGridColumn('sale_price', 'Preço', 'right', '15%');
        $col_unity       = new TDataGridColumn('unity', 'Categoria', 'right', '15%');
